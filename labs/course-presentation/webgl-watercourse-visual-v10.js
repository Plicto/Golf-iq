...
export const WEBGL_VISUAL_WATERCOURSE_VERSION = "tangent-stream-v10";
export const WEBGL_VISUAL_WATER_SURFACE_LIFT_METERS = 0.024;

const STREAM_STATIONS = 112;
const BANK_CONTACT_WIDTH_METERS = 0.9;
const BANK_EDGE_RISE_METERS = 0.006;
const BANK_OUTER_RISE_METERS = 0.045;
...
const buildVisualGroup = (world, gameplay) => {
  ...
  for (let index = 0; index < STREAM_STATIONS; index += 1) {
    ...
    const taper = 0.14 + endBlend * 0.86;
    const requested = Math.max(0.12, halfWidthAt(spec, primary) * taper);
...
const groundedSurfaceElevationAt = (world, group, waterHeight) => (x, z) => {
  const original = world.surfaceElevationAt(x, z);
  const point = { x, z };
  if (pointInCoursePolygon(group, point)) return original;
  const distance = distanceToBoundary(group, point);
  if (distance >= BANK_CONTACT_WIDTH_METERS) return original;
  const amount = smoothstep(distance / BANK_CONTACT_WIDTH_METERS);
  const contactHeight = waterHeight + BANK_EDGE_RISE_METERS +
    amount * BANK_OUTER_RISE_METERS;
  return Math.max(original, contactHeight);
};
...
export function createVisualWatercourseWorld(world) {
  ...
  const waterHeight = level + WEBGL_VISUAL_WATER_SURFACE_LIFT_METERS;
  ...
  return Object.freeze({
    ...
    surfaceElevationAt: groundedSurfaceElevationAt(world, points, waterHeight),
  });
}
