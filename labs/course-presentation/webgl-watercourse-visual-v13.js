...
export const WEBGL_VISUAL_WATERCOURSE_VERSION = "tangent-stream-v13";
export const WEBGL_VISUAL_WATER_SURFACE_LIFT_METERS = 0.005;

const STREAM_STATIONS = 128;
const TERRAIN_EDGE_DROP_METERS = 0.003;
const TERRAIN_BED_DEPTH_METERS = 0.11;
const TERRAIN_INNER_BLEND_METERS = 1.5;
const TERRAIN_OUTER_BLEND_METERS = 4.2;
...
const centerDerivativeAt = (spec, primary) => {
  ...
  const step = Math.max(0.05, (last - first) / 2200);
...
const fittedHalfWidth = (gameplay, frame, requested) => {
  let half = Math.max(0.32, requested);
...
const buildVisualGroup = (world, gameplay) => {
  ...
  for (let index = 0; index < STREAM_STATIONS; index += 1) {
    ...
    const endBlend = Math.min(
      smoothstep(progress / 0.09),
      smoothstep((1 - progress) / 0.09),
    );
    const taper = 0.58 + endBlend * 0.42;
    const requested = Math.max(0.38, halfWidthAt(spec, primary) * taper);
...
const insideTerrainHeightAt = (waterLevel, distance) => {
  const amount = smoothstep(distance / TERRAIN_INNER_BLEND_METERS);
  return waterLevel - mix(
...
const outsideTerrainHeightAt = (original, waterLevel, distance) => {
  ...
  const edgeHeight = waterLevel - TERRAIN_EDGE_DROP_METERS;
  return Math.min(original, mix(edgeHeight, original, amount));
};
  ...
  return Object.freeze({
    ...
    surfaceElevationAt: (x, z) => {
      ...
      for (let index = 0; index < groups.length; index += 1) {
        ...
        const candidate = pointInCoursePolygon(points, point)
          ? insideTerrainHeightAt(waterLevel, distance)
          : outsideTerrainHeightAt(original, waterLevel, distance);
        bestHeight = Math.min(bestHeight, candidate);
      }
      return bestHeight;
    },
  });
